...
package com.serpest.rebuk.utils.javafx;
...
/**
 * Loads an object hierarchy from an XML document. For more information, see the
 * <a href="doc-files/introduction_to_fxml.html">Introduction to FXML</a>
 * document.
 * 
 * The standard <code>FXMLLoader</code> can't work with controller proxies
 * because it uses reflection to directly access to the <code>@FXML</code>
 * annotated fields in the proxies and not in the target controller. This
 * implementation works only with proxies that implement
 * <code>org.springframework.aop.framework.Advised</code> (inner proxies don't
 * work).
 *
 * @since JavaFX 2.0
 */
@SuppressWarnings("all")
public class ControllerProxyFXMLLoader {
	...
	private abstract class Element {
		...
		public void processStartElement() throws IOException {
			for (int i = 0, n = xmlStreamReader.getAttributeCount(); i < n; i++) {
				...
				if (loadListener != null && prefix != null && prefix.equals(FX_NAMESPACE_PREFIX)) {
		...
		public void processAttribute(String prefix, String localName, String value) throws IOException {
			if (prefix == null) {
				...
				throw constructLoadException(prefix + ":" + localName + " is not a valid attribute.");
			}
		}
			...
			if (sourceType == null && isTyped()) {
				...
				if (type == null) {
					throw new PropertyNotFoundException(
							"Property \"" + propertyName + "\" does not exist" + " or is read-only.");
				}
		...
		private Object resolvePrefixedValue(String aValue) throws LoadException {
			if (aValue.startsWith(ESCAPE_PREFIX)) {
				...
				if (aValue.length() == 0 || !(aValue.startsWith(ESCAPE_PREFIX)
						|| aValue.startsWith(RELATIVE_PATH_PREFIX) || aValue.startsWith(RESOURCE_KEY_PREFIX)
						|| aValue.startsWith(EXPRESSION_PREFIX) || aValue.startsWith(BI_DIRECTIONAL_BINDING_PREFIX))) {
				...
				if (aValue.startsWith(RELATIVE_PATH_PREFIX)) {
					...
					if (aValue.charAt(0) == '/') {
						...
						try {
							return new URL(ControllerProxyFXMLLoader.this.location, aValue).toString();
						} catch (MalformedURLException e) {
							System.err.println(ControllerProxyFXMLLoader.this.location + "/" + aValue);
						}
					}
				}
		...
		/**
		 * Creates an array of given type and populates it with values from a string
		 * where tokens are separated by ARRAY_COMPONENT_DELIMITER. If token is prefixed
		 * with RELATIVE_PATH_PREFIX a value added to the array becomes relative to
		 * document location.
		 */
			...
			if (stringValue.length() > 0) {
				...
				for (int i = 0; i < values.length; i++) {
					Array.set(propertyValue, i,
							BeanAdapter.coerce(resolvePrefixedValue(values[i].trim()), type.getComponentType()));
				}
			} else {
		...
		/**
		 * Populates list with values from a string where tokens are separated by
		 * ARRAY_COMPONENT_DELIMITER. If token is prefixed with RELATIVE_PATH_PREFIX a
		 * value added to the list becomes relative to document location.
		 */
		private void populateListFromString(BeanAdapter valueAdapter, String listPropertyName, String stringValue)
			...
			if (stringValue.length() > 0) {
				...
				for (String aValue : values) {
					aValue = aValue.trim();
					list.add(BeanAdapter.coerce(resolvePrefixedValue(aValue), (Class<?>) itemType));
				}
			}
		}
		...
		public void warnDeprecatedEscapeSequence(String prefix) {
			System.err.println(
					prefix + prefix + " is a deprecated escape sequence. " + "Please use \\" + prefix + " instead.");
		}
		...
		private MethodHandler getControllerMethodHandle(String handlerName, SupportedType... types)
				throws LoadException {
			if (handlerName.startsWith(CONTROLLER_METHOD_PREFIX)) {
				...
				if (!handlerName.startsWith(CONTROLLER_METHOD_PREFIX)) {
					...
					for (SupportedType t : types) {
						Method method = controllerAccessor.getControllerMethods().get(t).get(handlerName);
						if (method != null) {
					...
					if (value instanceof ObservableList && attribute.name.equals(COLLECTION_HANDLER_NAME)) {
						...
						if (eventHandler == null) {
							if (handlerName.length() == 0 || scriptEngine == null) {
								throw constructLoadException("Error resolving " + attribute.name + "='"
										+ attribute.value
		...
		private void processPropertyHandler(String attributeName, String handlerValue) throws LoadException {
			...
			if (i != j) {
				...
				if (propertyModel == null) {
					throw constructLoadException(
							value.getClass().getName() + " does not define" + " a property model for \"" + key + "\".");
				}

				if (handlerValue.startsWith(CONTROLLER_METHOD_PREFIX)) {
					final MethodHandler handler = getControllerMethodHandle(handlerValue,
							SupportedType.PROPERTY_CHANGE_LISTENER, SupportedType.EVENT);
					if (handler != null) {
						if (handler.type == SupportedType.EVENT) {
							// Note: this part is solely for purpose of 2.2 backward compatibility where an
							// Event object
	...
	private abstract class ValueElement extends Element {
			...
			if (parent == null) {
				...
				if (fxNSURI != null) {
					...
					if (compareJFXVersions(FX_NAMESPACE_VERSION, fxVersion) < 0) {
						throw constructLoadException("Loading FXML document of version " + fxVersion
								+ " by JavaFX runtime supporting version " + FX_NAMESPACE_VERSION);
					}
				}
		...
		@Override
		public void processAttribute(String prefix, String localName, String value) throws IOException {
			if (prefix != null && prefix.equals(FX_NAMESPACE_PREFIX)) {
	...
	private class UnknownTypeElement extends ValueElement {
		...
		public class UnknownValueMap extends AbstractMap<String, Object> {
			...
			public Object get(Object key) {
				...
				return (key.equals(getClass().getAnnotation(DefaultProperty.class).value())) ? items : values.get(key);
			}
	...
	private class IncludeElement extends ValueElement {
		...
		public ResourceBundle resources = ControllerProxyFXMLLoader.this.resources;
		public Charset charset = ControllerProxyFXMLLoader.this.charset;
			...
			if (prefix == null) {
				if (localName.equals(INCLUDE_SOURCE_ATTRIBUTE)) {
					...
					resources = ResourceBundle.getBundle(value, Locale.getDefault(),
							ControllerProxyFXMLLoader.this.resources.getClass().getClassLoader());
		...
		public Object constructValue() throws IOException {
			...
			if (source.charAt(0) == '/') {
				...
				if (ControllerProxyFXMLLoader.this.location == null) {
					throw constructLoadException("Base location is undefined.");
				}

				location = new URL(ControllerProxyFXMLLoader.this.location, source);
			}

			ControllerProxyFXMLLoader fxmlLoader = new ControllerProxyFXMLLoader(location, resources, builderFactory,
					controllerFactory, charset, loaders);
			fxmlLoader.parentLoader = ControllerProxyFXMLLoader.this;

			if (isCyclic(ControllerProxyFXMLLoader.this, fxmlLoader)) {
				throw new IOException(String.format("Including \"%s\" in \"%s\" created cyclic reference.",
						fxmlLoader.location.toExternalForm(),
						ControllerProxyFXMLLoader.this.location.toExternalForm()));
			}
	...
	private void injectFields(String fieldName, Object value) throws LoadException {
		if (controller != null && fieldName != null) {
			Object targetController = null;
			try {
				targetController = ((Advised) controller).getTargetSource().getTarget();
		...
		public Object constructValue() throws LoadException {
			...
			if (root == null) {
				...
				if (!type.isAssignableFrom(root.getClass())) {
					throw constructLoadException("Root is not an instance of " + type.getName() + ".");
				}
	...
	private class ScriptElement extends Element {
		...
		public void processStartElement() throws IOException {
			...
			if (source != null && !staticLoad) {
				...
				if (i == -1) {
					throw constructLoadException("Cannot determine type of script \"" + source + "\".");
				}
				...
				if (scriptEngine != null && scriptEngine.getFactory().getExtensions().contains(extension)) {
					// If we have a page language and it's engine supports the extension, use the
					// same engine
					engine = scriptEngine;
				} else {
				...
				if (engine == null) {
					throw constructLoadException(
							"Unable to locate scripting engine for" + " extension " + extension + ".");
				}
		...
		@Override
		public void processAttribute(String prefix, String localName, String value) throws IOException {
			if (prefix == null && localName.equals(SCRIPT_SOURCE_ATTRIBUTE)) {
				if (loadListener != null) {
					loadListener.readInternalAttribute(localName, value);
				}
				...
				throw constructLoadException(
						prefix == null ? localName : prefix + ":" + localName + " is not a valid attribute.");
			}
		}
	}
	...
	private static class ScriptEventHandler implements EventHandler<Event> {
		...
		public ScriptEventHandler(String script, ScriptEngine scriptEngine, String filename) {
			...
			if (scriptEngine instanceof Compilable && compileScript) {
				try {
					// supply the filename to the scriptEngine engine scope Bindings in case it is
	...
	private final LinkedList<ControllerProxyFXMLLoader> loaders;

	private ClassLoader classLoader = null;
	private boolean staticLoad = false;
	private LoadListener loadListener = null;

	private ControllerProxyFXMLLoader parentLoader;
	...
	/**
	 * Escape prefix for escaping special characters inside attribute values. Serves
	 * as an escape for {@link #ESCAPE_PREFIX}, {@link #RELATIVE_PATH_PREFIX},
	...
	/**
	 * A suffix for controllers of included fxml files. The full key is stored in
	 * namespace map.
	 * 
	 * @see #getNamespace()
	 * @since JavaFX 2.2
	 ...
	 */
	public ControllerProxyFXMLLoader() {
		this((URL) null);
	}
	 ...
	 * @since JavaFX 2.1
	 */
	public ControllerProxyFXMLLoader(URL location) {
		this(location, null);
	}
	 ...
	 * @since JavaFX 2.1
	 */
	public ControllerProxyFXMLLoader(URL location, ResourceBundle resources) {
	 ...
	 * Creates a new FXMLLoader instance.
	 *
	 * @param location       the location used to resolve relative path attribute
	 *                       values
	 ...
	 * @since JavaFX 2.1
	 */
	public ControllerProxyFXMLLoader(URL location, ResourceBundle resources, BuilderFactory builderFactory) {
	 ...
	 * @param location          the location used to resolve relative path attribute
	 *                          values
	 * @param resources         resources used to resolve resource key attribute
	 *                          values
	 * @param builderFactory    the builder factory used by this loader
	 ...
	 */
	public ControllerProxyFXMLLoader(Charset charset) {
	...
	public ControllerProxyFXMLLoader(URL location, ResourceBundle resources, BuilderFactory builderFactory,
		...
		this(location, resources, builderFactory, controllerFactory, charset,
				new LinkedList<ControllerProxyFXMLLoader>());
	}
	...
	public ControllerProxyFXMLLoader(URL location, ResourceBundle resources, BuilderFactory builderFactory,
			Callback<Class<?>, Object> controllerFactory, Charset charset,
			LinkedList<ControllerProxyFXMLLoader> loaders) {
	...
	/**
	 * Sets the root of the object hierarchy. The value passed to this method is
	 * used as the value of the {@code <fx:root>} tag. This method must be called
	 * prior to loading the document when using {@code <fx:root>}.
	 *
	...
	public boolean equals(Object obj) {
		if (obj instanceof ControllerProxyFXMLLoader) {
			ControllerProxyFXMLLoader loader = (ControllerProxyFXMLLoader) obj;
			if (location == null || loader.location == null) {
				return loader.location == location;
			}
			return location.toExternalForm().equals(loader.location.toExternalForm());
		}
		return false;
	}

	private boolean isCyclic(ControllerProxyFXMLLoader currentLoader, ControllerProxyFXMLLoader node) {
	...
	/**
	 * Sets the controller associated with the root object. The value passed to this
	 * method is used as the value of the {@code fx:controller} attribute. This
	 * method must be called prior to loading the document when using controller
	 * event handlers when an {@code fx:controller} attribute is not specified in
	 * the document.
	 *
	...
	/**
	 * Loads an object hierarchy from a FXML document. The location from which the
	 * document will be loaded must have been set by a prior call to
	...
	public <T> T load() throws IOException {
		return loadImpl((System.getSecurityManager() != null) ? walker.getCallerClass() : null);
	}
	...
	public <T> T load(InputStream inputStream) throws IOException {
		return loadImpl(inputStream, (System.getSecurityManager() != null) ? walker.getCallerClass() : null);
	}
		...
		try {
			...
			if (controller != null) {
				if (controller instanceof Initializable) {
					...
					Map<String, List<Field>> controllerFields = controllerAccessor.getControllerFields();
	...
	private String constructFXMLTrace() {
		...
		for (ControllerProxyFXMLLoader loader : loaders) {
	...
	ParseTraceElement[] getParseTrace() {
		...
		int i = 0;
		for (ControllerProxyFXMLLoader loader : loaders) {
			parseTrace[i++] = new ParseTraceElement(loader.location,
					(loader.current != null) ? loader.getLineNumber() : -1);
		}
	...
	private void processProcessingInstruction() throws LoadException {
		...
		if (piTarget.equals(LANGUAGE_PROCESSING_INSTRUCTION)) {
			...
			// if PIData() is empty string then default to true, otherwise use
			// Boolean.parseBoolean(string) to determine the boolean value
	...
	private static boolean needsClassLoaderPermissionCheck(Class caller) {
		...
		return !ControllerProxyFXMLLoader.class.getModule().equals(caller.getModule());
	}
	 ...
	 *
	 * @param defaultClassLoader The default class loader to use when loading
	 *                           classes.
	 * @since JavaFX 2.1
	 */
	public static void setDefaultClassLoader(ClassLoader defaultClassLoader) {
		...
		ControllerProxyFXMLLoader.defaultClassLoader = defaultClassLoader;
	}
	...
	public static <T> T load(URL location) throws IOException {
		return loadImpl(location, (System.getSecurityManager() != null) ? walker.getCallerClass() : null);
	}
	...
	public static <T> T load(URL location, ResourceBundle resources) throws IOException {
		return loadImpl(location, resources, (System.getSecurityManager() != null) ? walker.getCallerClass() : null);
	}

	private static <T> T loadImpl(URL location, ResourceBundle resources, Class<?> callerClass) throws IOException {
		return loadImpl(location, resources, null, callerClass);
	}
	 ...
	 * @param location       the location used to resolve relative path attribute
	 *                       values
	 * @param resources      the resources used to resolve resource key attribute
	 *                       values
	...
	public static <T> T load(URL location, ResourceBundle resources, BuilderFactory builderFactory) throws IOException {
		return loadImpl(location, resources, builderFactory,
				(System.getSecurityManager() != null) ? walker.getCallerClass() : null);
	}

	private static <T> T loadImpl(URL location, ResourceBundle resources, BuilderFactory builderFactory,
	 ...
	 * @param controllerFactory the controller factory used when loading the
	 *                          document
	 *
	 * @throws IOException if an error occurs during loading
	 * @return the loaded object hierarchy
	...
	private static <T> T loadImpl(URL location, ResourceBundle resources, BuilderFactory builderFactory,
			Callback<Class<?>, Object> controllerFactory, Class<?> callerClass) throws IOException {
		return loadImpl(location, resources, builderFactory, controllerFactory, Charset.forName(DEFAULT_CHARSET_NAME),
				callerClass);
	}
	...
	public static <T> T load(URL location, ResourceBundle resources, BuilderFactory builderFactory,
		...
		return loadImpl(location, resources, builderFactory, controllerFactory, charset,
				(System.getSecurityManager() != null) ? walker.getCallerClass() : null);
	}

	private static <T> T loadImpl(URL location, ResourceBundle resources, BuilderFactory builderFactory,
			Callback<Class<?>, Object> controllerFactory, Charset charset, Class<?> callerClass) throws IOException {
		if (location == null) {
	...
	/**
	 * Utility method for comparing two JavaFX version strings (such as 2.2.5,
	 * 8.0.0-ea)
	 * 
	 * @param rtVer String representation of JavaFX runtime version, including - or
	 *              _ appendix
	 * @param nsVer String representation of JavaFX version to compare against
	 *              runtime version
	 * @return number &lt; 0 if runtime version is lower, 0 when both versions are
	 *         the same, number &gt; 0 if runtime is higher version
	 */
	...
	private static final class ControllerAccessor {
		...
		private static final int INITIAL_CLASS_ACCESS = PUBLIC | PROTECTED | PACKAGE | PRIVATE;
		...
		Map<String, List<Field>> getControllerFields() {
			if (controllerFields == null) {
				...
				addAccessibleMembers(targetController.getClass(), INITIAL_CLASS_ACCESS, INITIAL_MEMBER_ACCESS, FIELDS);
			}
		...
		private void addAccessibleMembers(final Class<?> type, final int prevAllowedClassAccess,
				final int prevAllowedMemberAccess, final int membersType) {
		...
		private void addAccessibleFields(final Class<?> type, final int allowedMemberAccess) {
			...
			for (int i = 0; i < fields.length; ++i) {
				...
				if (((memberModifiers & (Modifier.STATIC | Modifier.FINAL)) != 0)
						|| ((getAccess(memberModifiers) & allowedMemberAccess) == 0)) {
					continue;
				}
